...
class ServerTransformer extends BaseClientTransformer
{
    ...
    public function transform(Server $server): array
    {
        ...
        return [
            ...
            'node_alert' => $server->node?->alert ?? null,
            'node_icon' => $server->node?->node_icon ?? null,
            'daemon_text' => $server->node?->daemon_text ?? null,
            'container_text' => $server->node?->container_text ?? null,
            'egg_image' => $server->egg?->image ?? null,
            'egg_name' => $server->egg?->name ?? null,
            'nest_id' => $server->nest_id,
            'egg_id' => $server->egg_id,
            'is_node_under_maintenance' => $server->node ? $server->node->isUnderMaintenance() : false,
            'sftp_details' => [
                'ip' => $server->node?->fqdn ?? '',
